...
describe('<Sidebar />', () => {
  ...
  it('Should render the correct label and the amount next to it', () => {
    expect(wrapper.find('Label').at(0).render().text()).toBe('Cats (2)');
    expect(wrapper.find('Label').at(1).render().text()).toBe('Lions (1)');
    expect(wrapper.find('Label').at(2).render().text()).toBe('Dogs (1)');
  });
